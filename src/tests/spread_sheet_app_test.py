...
class TestSpreadSheetApp(unittest.TestCase):
    ...
    def test_app_saves_cells_correctly(self):
        self.app.saveStateToFile()
        try:
            file = open("test.json")
            data = json.load(file)
            cellValues = data["cell_values"]
            self.assertEqual(cellValues["1"], 0)
        except:
            self.assertEqual("something failed when trying to read save file", False)
    
    def test_app_saves_edited_cells_correctly(self):
        self.app.spread_sheet_view.cell_grid_values[1] = "This is a test"
        self.app.saveStateToFile()
        try:
            file = open("test.json")
            data = json.load(file)
            cellValues = data["cell_values"]
            self.assertEqual(cellValues["1"], "This is a test")
